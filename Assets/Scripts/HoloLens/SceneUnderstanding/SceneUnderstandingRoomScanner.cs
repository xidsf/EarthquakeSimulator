using System;
using System.Collections;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using Microsoft.MixedReality.SceneUnderstanding;
using UnityEngine;
using UnityEngine.Events;

using NumVector3 = System.Numerics.Vector3;
using NumMatrix4x4 = System.Numerics.Matrix4x4;
using UVector2 = UnityEngine.Vector2;
using UVector3 = UnityEngine.Vector3;
using UMatrix4x4 = UnityEngine.Matrix4x4;

#if MSFT_OPENXR
using Microsoft.MixedReality.OpenXR;
#endif

public partial class SceneUnderstandingRoomScanner : MonoBehaviour
{
    [Header("Scan")]
    public bool autoUpdate = true;
    public bool lockRoom = false;
    public float updateIntervalSeconds = 2.0f;
    public float queryRadiusMeters = 8.0f;
    public SceneMeshLevelOfDetail meshLevelOfDetail = SceneMeshLevelOfDetail.Medium;

    [Header("Workflow Guard")]
    public RoomBuildWorkflowManager workflowManager;
    public bool autoFindWorkflowManager = true;
    public bool requireRoomBuildStateToScan = true;

    [Header("Room Reconstruction")]
    public float minWallHeightMeters = 1.2f;
    public float minWallLengthMeters = 0.5f;
    public float maxCloseGapMeters = 0.45f;
    public bool createClosureEdges = true;
    public bool showClosureEdges = true;
    public bool attachSourceMeshDebugChild = true;

    [Header("Wall Candidate Filter")]
    public bool enableWallCandidateFiltering = true;

    [Tooltip("Wall mesh projected XZ thickness above this value is not accepted as an automatic DetectedWall.")]
    [Min(0.01f)] public float maxWallThicknessMeters = 0.30f;

    [Tooltip("Length/thickness ratio required before a Scene Understanding wall mesh is accepted automatically.")]
    [Min(1.0f)] public float minWallAspectRatio = 3.0f;

    [Tooltip("PCA direction confidence required before a Scene Understanding wall mesh is accepted automatically.")]
    [Range(0.0f, 1.0f)] public float minWallDirectionConfidence = 0.45f;

    [Tooltip("If the wall bottom is farther than this from the detected floor, it is kept as a candidate instead of an automatic wall.")]
    [Min(0.0f)] public float maxWallBottomFloorGapMeters = 0.35f;

    [Tooltip("If the wall top is farther than this from the detected ceiling, it is kept as a candidate instead of an automatic wall.")]
    [Min(0.0f)] public float maxWallTopCeilingGapMeters = 0.60f;

    [Tooltip("If an ActiveWallCandidate is fully covered by a stronger DetectedWall, move it to IgnoredWallCandidate.")]
    public bool ignoreActiveCandidatesContainedInDetectedWalls = true;

    [Tooltip("Maximum angle difference used when checking if an ActiveWallCandidate is inside a DetectedWall.")]
    [Range(0.0f, 45.0f)] public float containedCandidateMaxAngleDegrees = 10.0f;

    [Tooltip("Maximum perpendicular distance used when checking if an ActiveWallCandidate is inside a DetectedWall.")]
    [Min(0.0f)] public float containedCandidateMaxDistanceMeters = 0.20f;

    [Tooltip("Endpoint projection padding used when checking if an ActiveWallCandidate is inside a DetectedWall.")]
    [Min(0.0f)] public float containedCandidateEndpointPaddingMeters = 0.05f;

    [Header("Visualization")]
    public Transform sceneRoot;
    public Material wallMaterial;
    public Material floorMaterial;
    public Material ceilingMaterial;
    public Material closureEdgeMaterial;
    public Material activeWallCandidateMaterial;
    public Material ignoredWallCandidateMaterial;

    [Tooltip("Raw Scene Understanding mesh는 화면에 출력하지 않습니다. 내부 계산만 수행합니다.")]
    public bool showRawSceneMeshes = false;

    public bool showDetectedWallSegments = true;
    public bool showReconstructedFloor = true;
    public bool showReconstructedCeiling = true;
    public bool showActiveWallCandidates = false;
    public bool showIgnoredWallCandidates = false;

    [Header("Runtime Debug")]
    public bool sceneRootAligned;
    public string sceneRootAlignStatus;
    public float lastComputeTimeSeconds;
    public float lastCameraY;
    public float lastFloorWorldY;
    public float lastEyeToFloor;

    [Header("Events")]
    public UnityEvent onSceneUpdated;
    public UnityEvent onRoomLocked;
    public UnityEvent onRoomUnlocked;

    public Scene currentScene { get; private set; }
    public RoomBuildResult currentRoom { get; private set; }

    private bool isRunning;
    private bool isComputing;
    private Scene previousScene;
    private GameObject rawRoot;
    private GameObject roomRoot;
    private Material runtimeActiveWallCandidateMaterial;
    private Material runtimeIgnoredWallCandidateMaterial;

    public Transform RawMeshRootTransform => rawRoot != null ? rawRoot.transform : null;

    private readonly List<ExtractedSceneMesh> extractedSceneMeshes = new List<ExtractedSceneMesh>();

    private class ExtractedSceneMesh
    {
        public SceneObjectKind kind;
        public List<UVector3> worldVertices = new List<UVector3>();
        public Mesh sourceMesh;
        public UMatrix4x4 localToWorldMatrix;
        public string sourceName;
    }

    private async void Start()
    {
        EnsureWorkflowManagerReference();

        if (sceneRoot == null)
        {
            sceneRoot = new GameObject("SceneUnderstandingRoot").transform;
        }

        await InitializeSceneUnderstanding();

        if (autoUpdate && CanScanForCurrentWorkflowState())
        {
            StartScanning();
        }
    }

    public void StartScanning()
    {
        EnsureWorkflowManagerReference();

        if (!CanScanForCurrentWorkflowState())
        {
            isRunning = false;
            return;
        }

        if (isRunning)
        {
            return;
        }

        isRunning = true;
        StartCoroutine(UpdateLoop());
    }

    public void StopScanning()
    {
        isRunning = false;
    }

    public void LockRoom()
    {
        lockRoom = true;
        onRoomLocked?.Invoke();
    }

    public void UnlockRoom()
    {
        lockRoom = false;
        onRoomUnlocked?.Invoke();
    }

    public async void ForceUpdate()
    {
        EnsureWorkflowManagerReference();

        if (lockRoom || !CanScanForCurrentWorkflowState())
        {
            return;
        }

        await ComputeAndDisplayScene();
    }

    private IEnumerator UpdateLoop()
    {
        while (isRunning)
        {
            if (!CanScanForCurrentWorkflowState())
            {
                isRunning = false;
                yield break;
            }

            if (!lockRoom && !isComputing)
            {
                _ = ComputeAndDisplayScene();
            }

            yield return new WaitForSeconds(updateIntervalSeconds);
        }
    }

    private async Task InitializeSceneUnderstanding()
    {
        if (!SceneObserver.IsSupported())
        {
            Debug.LogError("[SU] Scene Understanding is not supported on this device/runtime.");
            return;
        }

        SceneObserverAccessStatus accessStatus = await SceneObserver.RequestAccessAsync();
        Debug.Log($"[SU] Access status: {accessStatus}");
    }

    private async Task ComputeAndDisplayScene()
    {
        if (isComputing || !CanScanForCurrentWorkflowState())
        {
            return;
        }

        isComputing = true;
        float startTime = Time.realtimeSinceStartup;

        try
        {
            SceneQuerySettings settings = new SceneQuerySettings
            {
                EnableSceneObjectQuads = true,
                EnableSceneObjectMeshes = true,
                EnableWorldMesh = true,
                EnableOnlyObservedSceneObjects = false,
                RequestedMeshLevelOfDetail = meshLevelOfDetail
            };

            Scene newScene;

            if (previousScene != null)
            {
                newScene = await SceneObserver.ComputeAsync(settings, queryRadiusMeters, previousScene);
            }
            else
            {
                newScene = await SceneObserver.ComputeAsync(settings, queryRadiusMeters);
            }

            lastComputeTimeSeconds = Time.realtimeSinceStartup - startTime;

            if (newScene == null)
            {
                Debug.LogWarning("[SU] Computed scene is null.");
                return;
            }

            if (lockRoom || !CanScanForCurrentWorkflowState())
            {
                return;
            }

            previousScene = newScene;
            currentScene = newScene;

            if (!TryAlignSceneRoot(newScene))
            {
                Debug.LogWarning("[SU] Failed to align scene objects.");
            }

            ClearChildren();

            // Raw Scene Understanding mesh GameObject는 생성하지 않는다.
            // Wall/Floor/Ceiling SceneObject의 vertex만 내부적으로 추출해 재구성 모델을 만든다.
            ExtractSceneObjects(newScene);

            currentRoom = BuildRoomFromExtractedMeshes();
            DisplayRoomBuildResult(currentRoom);

            UpdateCameraFloorDebugValues();
            LogSceneSummary(newScene, currentRoom);

            onSceneUpdated?.Invoke();
        }
        catch (Exception ex)
        {
            Debug.LogError($"[SU] Compute failed: {ex}");
        }
        finally
        {
            isComputing = false;
        }
    }

    private bool CanScanForCurrentWorkflowState()
    {
        if (!requireRoomBuildStateToScan)
        {
            return true;
        }

        return workflowManager != null &&
               workflowManager.currentState == RoomBuildWorkflowManager.WorkflowState.RoomBuild;
    }

    private void EnsureWorkflowManagerReference()
    {
        if (workflowManager != null || !autoFindWorkflowManager)
        {
            return;
        }

        RoomBuildWorkflowManager[] managers =
            FindObjectsByType<RoomBuildWorkflowManager>(FindObjectsInactive.Include, FindObjectsSortMode.None);

        if (managers != null && managers.Length > 0)
        {
            workflowManager = managers[0];
        }
    }

    private bool TryAlignSceneRoot(Scene scene)
    {
        sceneRootAligned = false;
        sceneRootAlignStatus = "not tried";

        if (sceneRoot == null)
        {
            sceneRootAlignStatus = "sceneRoot is null";
            Debug.LogWarning("[SU] Scene root is null.");
            return false;
        }

#if MSFT_OPENXR
        if (scene == null)
        {
            sceneRootAlignStatus = "scene is null";
            Debug.LogWarning("[SU] Scene is null.");
            return false;
        }

        SpatialGraphNode node = SpatialGraphNode.FromStaticNodeId(scene.OriginSpatialGraphNodeId);

        if (node == null)
        {
            sceneRootAlignStatus = "SpatialGraphNode is null";
            Debug.LogWarning("[SU] Failed to create SpatialGraphNode from OriginSpatialGraphNodeId.");
            return false;
        }

        if (node.TryLocate(FrameTime.OnUpdate, out Pose pose))
        {
            sceneRoot.SetPositionAndRotation(pose.position, pose.rotation);

            sceneRootAligned = true;
            sceneRootAlignStatus =
                $"aligned pos:{pose.position.x:F2},{pose.position.y:F2},{pose.position.z:F2}";

            Debug.Log($"[SU] Scene root aligned. {sceneRootAlignStatus}");
            return true;
        }

        sceneRootAlignStatus = "TryLocate failed";
        Debug.LogWarning("[SU] SpatialGraphNode.TryLocate failed.");
        return false;
#else
        sceneRootAlignStatus = "MSFT_OPENXR not defined";
        Debug.LogWarning("[SU] MSFT_OPENXR is not defined. Scene root cannot be aligned through OpenXR SpatialGraphNode.");
        return false;
#endif
    }

    private void ClearChildren()
    {
        DestroyExistingGeneratedSceneChildren();
        extractedSceneMeshes.Clear();

        roomRoot = new GameObject("Reconstructed Room");
        roomRoot.transform.SetParent(sceneRoot, false);
    }

    private void DestroyExistingGeneratedSceneChildren()
    {
        if (rawRoot != null)
        {
            Destroy(rawRoot);
            rawRoot = null;
        }

        if (roomRoot != null)
        {
            Destroy(roomRoot);
            roomRoot = null;
        }

        if (sceneRoot == null)
        {
            return;
        }

        // 이전 버전에서 생성된 Raw Scene Understanding mesh가 private field 연결 없이
        // sceneRoot 아래에 남아 있으면, 다음 실행에서도 구멍난 raw mesh가 계속 보일 수 있다.
        // 이름 기준으로 기존 생성물을 모두 제거하고 재구성 모델만 다시 만든다.
        for (int i = sceneRoot.childCount - 1; i >= 0; i--)
        {
            Transform child = sceneRoot.GetChild(i);

            if (child == null)
            {
                continue;
            }

            if (child.name == "Raw Scene Understanding Objects" ||
                child.name == "Reconstructed Room")
            {
                Destroy(child.gameObject);
            }
        }
    }

    private void ExtractSceneObjects(Scene scene)
    {
        extractedSceneMeshes.Clear();

        if (scene == null)
        {
            return;
        }

        foreach (SceneObject sceneObject in scene.SceneObjects)
        {
            bool exportableKind =
                sceneObject.Kind == SceneObjectKind.Wall ||
                sceneObject.Kind == SceneObjectKind.Floor ||
                sceneObject.Kind == SceneObjectKind.Ceiling;

            if (!exportableKind)
            {
                continue;
            }

            UMatrix4x4 objectMatrix =
                ConvertRightHandedMatrix4x4ToLeftHanded(sceneObject.GetLocationAsMatrix());

            foreach (SceneMesh sceneMesh in sceneObject.Meshes)
            {
                Mesh unityMesh = GenerateUnityMesh(sceneMesh);

                if (unityMesh == null)
                {
                    continue;
                }

                List<UVector3> worldVertices = GetWorldVerticesFromSceneMesh(unityMesh, objectMatrix);

                if (worldVertices.Count == 0)
                {
                    continue;
                }

                extractedSceneMeshes.Add(new ExtractedSceneMesh
                {
                    kind = sceneObject.Kind,
                    worldVertices = worldVertices,
                    sourceMesh = unityMesh,
                    localToWorldMatrix = GetSceneMeshWorldMatrix(objectMatrix),
                    sourceName = $"{sceneObject.Kind}_SourceMesh_{extractedSceneMeshes.Count}"
                });
            }
        }
    }

    // Raw mesh 출력은 제거했다. Debug UI 호환을 위해 빈 함수로 유지한다.
    private void DisplayRawSceneObjects(Scene scene)
    {
        ExtractSceneObjects(scene);
    }

    private void SetRawMeshRenderersVisible(bool visible)
    {
        // Raw Scene Understanding mesh GameObject는 더 이상 생성하지 않는다.
    }

    private RoomBuildResult BuildRoomFromExtractedMeshes()
    {
        RoomBuildResult result = new RoomBuildResult();

        List<WallSegment> rawSegments = new List<WallSegment>();
        List<WallSegment> extractedWallSegments = new List<WallSegment>();
        List<float> floorYValues = new List<float>();
        List<float> ceilingYValues = new List<float>();

        for (int extractedIndex = 0; extractedIndex < extractedSceneMeshes.Count; extractedIndex++)
        {
            ExtractedSceneMesh extracted = extractedSceneMeshes[extractedIndex];
            List<UVector3> points = extracted.worldVertices;

            if (points == null || points.Count == 0)
            {
                continue;
            }

            if (extracted.kind == SceneObjectKind.Floor)
            {
                floorYValues.AddRange(points.Select(p => p.y));
            }
            else if (extracted.kind == SceneObjectKind.Ceiling)
            {
                ceilingYValues.AddRange(points.Select(p => p.y));
            }
            else if (extracted.kind == SceneObjectKind.Wall)
            {
                WallSegment? segment = TryExtractWallSegmentFromWorldPoints(points);

                if (segment.HasValue)
                {
                    WallSegment wallSegment = segment.Value;
                    wallSegment.sourceMeshIndex = extractedIndex;
                    wallSegment.sourceMeshName = extracted.sourceName;
                    extractedWallSegments.Add(wallSegment);
                }
            }
        }

        result.floorY = floorYValues.Count > 0 ? Median(floorYValues) : float.NaN;
        result.ceilingY = ceilingYValues.Count > 0 ? Median(ceilingYValues) : float.NaN;

        if (float.IsNaN(result.floorY) || float.IsNaN(result.ceilingY))
        {
            Debug.LogError("[Room] Failed to calculate floor or ceiling Y from Scene Understanding vertices.");
            return result;
        }

        if (result.ceilingY <= result.floorY)
        {
            Debug.LogError(
                $"[Room] Invalid floor/ceiling order. " +
                $"floorY:{result.floorY:F2}, ceilingY:{result.ceilingY:F2}. " +
                $"This indicates a coordinate extraction or scene object classification issue."
            );
        }

        foreach (WallSegment segment in extractedWallSegments)
        {
            WallSegment s = segment;
            WallCandidateState state = EvaluateWallCandidate(ref s, result.floorY, result.ceilingY);
            s.candidateState = state;

            if (state == WallCandidateState.Accepted)
            {
                rawSegments.Add(s);
            }
            else if (state == WallCandidateState.ActiveCandidate)
            {
                result.activeWallCandidates.Add(s);
            }
            else if (state == WallCandidateState.PassiveCandidate)
            {
                result.passiveWallCandidates.Add(s);
            }
            else
            {
                result.ignoredWallCandidates.Add(s);
            }
        }

        result.detectedWalls = MergeSimilarWallSegments(rawSegments);
        MoveContainedActiveCandidatesToIgnored(result);

        if (createClosureEdges)
        {
            result.closureEdges = BuildClosureEdges(
                result.detectedWalls,
                maxCloseGapMeters,
                result.floorY,
                result.ceilingY
            );
        }

        result.isClosedEnough = IsClosedEnough(
            result.detectedWalls,
            result.closureEdges,
            maxCloseGapMeters
        );

        return result;
    }

    // 이전 SimpleDebugUI 파일과의 호환성을 위해 유지.
    // 새 계산은 BuildRoomFromExtractedMeshes()가 기준이다.
    private RoomBuildResult BuildRoomFromScene(Scene scene)
    {
        if (scene == null)
        {
            return new RoomBuildResult();
        }

        if (extractedSceneMeshes.Count == 0)
        {
            ExtractSceneObjects(scene);
        }

        return BuildRoomFromExtractedMeshes();
    }

    public void RefreshCurrentRoomClosureState()
    {
        if (currentRoom == null)
        {
            return;
        }

        currentRoom.isClosedEnough = IsClosedEnough(
            currentRoom.detectedWalls,
            currentRoom.closureEdges,
            maxCloseGapMeters
        );

    }

    private List<UVector3> GetWorldVerticesFromMeshFilter(MeshFilter meshFilter)
    {
        List<UVector3> points = new List<UVector3>();

        if (meshFilter == null || meshFilter.sharedMesh == null)
        {
            return points;
        }

        UVector3[] vertices = meshFilter.sharedMesh.vertices;
        Transform meshTransform = meshFilter.transform;

        for (int i = 0; i < vertices.Length; i++)
        {
            points.Add(meshTransform.TransformPoint(vertices[i]));
        }

        return points;
    }

    private List<UVector3> GetWorldVerticesFromSceneMesh(Mesh mesh, UMatrix4x4 objectMatrix)
    {
        List<UVector3> points = new List<UVector3>();

        if (mesh == null)
        {
            return points;
        }

        UVector3[] vertices = mesh.vertices;

        for (int i = 0; i < vertices.Length; i++)
        {
            UVector3 localPoint = objectMatrix.MultiplyPoint3x4(vertices[i]);

            if (sceneRoot != null)
            {
                points.Add(sceneRoot.TransformPoint(localPoint));
            }
            else
            {
                points.Add(localPoint);
            }
        }

        return points;
    }

    private UMatrix4x4 GetSceneMeshWorldMatrix(UMatrix4x4 objectMatrix)
    {
        if (sceneRoot != null)
        {
            return sceneRoot.localToWorldMatrix * objectMatrix;
        }

        return objectMatrix;
    }

    private WallSegment? TryExtractWallSegmentFromWorldPoints(List<UVector3> points)
    {
        if (points == null || points.Count < 4)
        {
            return null;
        }

        float minY = points.Min(p => p.y);
        float maxY = points.Max(p => p.y);
        float height = maxY - minY;

        List<UVector2> xz = points
            .Select(p => new UVector2(p.x, p.z))
            .ToList();

        UVector2 center = Average(xz);

        float xx = 0.0f;
        float xzCov = 0.0f;
        float zz = 0.0f;

        foreach (UVector2 p in xz)
        {
            UVector2 d = p - center;
            xx += d.x * d.x;
            xzCov += d.x * d.y;
            zz += d.y * d.y;
        }

        float angle = 0.5f * Mathf.Atan2(2.0f * xzCov, xx - zz);
        UVector2 dir = new UVector2(Mathf.Cos(angle), Mathf.Sin(angle)).normalized;
        UVector2 normal = new UVector2(-dir.y, dir.x);

        float covarianceScale = Mathf.Max(1, xz.Count);
        float normalizedXx = xx / covarianceScale;
        float normalizedXz = xzCov / covarianceScale;
        float normalizedZz = zz / covarianceScale;
        float trace = normalizedXx + normalizedZz;
        float determinant = normalizedXx * normalizedZz - normalizedXz * normalizedXz;
        float discriminant = Mathf.Sqrt(Mathf.Max(0.0f, trace * trace * 0.25f - determinant));
        float lambdaA = trace * 0.5f + discriminant;
        float lambdaB = trace * 0.5f - discriminant;
        float directionConfidence = lambdaA > 0.00001f
            ? Mathf.Clamp01(1.0f - Mathf.Max(0.0f, lambdaB) / lambdaA)
            : 0.0f;

        float minT = float.MaxValue;
        float maxT = float.MinValue;
        float minN = float.MaxValue;
        float maxN = float.MinValue;

        foreach (UVector2 p in xz)
        {
            UVector2 d = p - center;
            float t = UVector2.Dot(d, dir);
            float n = UVector2.Dot(d, normal);
            minT = Mathf.Min(minT, t);
            maxT = Mathf.Max(maxT, t);
            minN = Mathf.Min(minN, n);
            maxN = Mathf.Max(maxN, n);
        }

        UVector2 a = center + dir * minT;
        UVector2 b = center + dir * maxT;
        float length = UVector2.Distance(a, b);
        float thickness = Mathf.Max(0.0f, maxN - minN);

        return new WallSegment
        {
            start = a,
            end = b,
            center = center,
            bottomY = minY,
            topY = maxY,
            height = height,
            xzThickness = thickness,
            aspectRatio = length / Mathf.Max(0.001f, thickness),
            directionConfidence = directionConfidence,
            candidateState = WallCandidateState.Accepted,
            qualityReason = string.Empty,
            sourceMeshIndex = -1,
            sourceMeshName = string.Empty,
            isClosure = false
        };
    }

    private WallCandidateState EvaluateWallCandidate(ref WallSegment segment, float floorY, float ceilingY)
    {
        if (!enableWallCandidateFiltering)
        {
            segment.qualityReason = "Filtering disabled.";
            return WallCandidateState.Accepted;
        }

        List<string> reasons = new List<string>();

        if (segment.length < minWallLengthMeters)
        {
            reasons.Add($"short:{segment.length:F2}m");
        }

        if (segment.height < minWallHeightMeters)
        {
            reasons.Add($"low:{segment.height:F2}m");
        }

        if (reasons.Count > 0)
        {
            segment.qualityReason = string.Join(", ", reasons);
            return WallCandidateState.IgnoredCandidate;
        }

        bool activeCandidate = false;

        if (segment.xzThickness > maxWallThicknessMeters)
        {
            activeCandidate = true;
            reasons.Add($"thick:{segment.xzThickness:F2}m");
        }

        if (segment.aspectRatio < minWallAspectRatio)
        {
            activeCandidate = true;
            reasons.Add($"aspect:{segment.aspectRatio:F1}");
        }

        if (segment.directionConfidence < minWallDirectionConfidence)
        {
            activeCandidate = true;
            reasons.Add($"direction:{segment.directionConfidence:F2}");
        }

        if (!float.IsNaN(floorY))
        {
            float bottomGap = Mathf.Abs(segment.bottomY - floorY);

            if (bottomGap > maxWallBottomFloorGapMeters)
            {
                activeCandidate = true;
                reasons.Add($"floorGap:{bottomGap:F2}m");
            }
        }

        if (!float.IsNaN(ceilingY))
        {
            float topGap = Mathf.Abs(ceilingY - segment.topY);

            if (topGap > maxWallTopCeilingGapMeters)
            {
                activeCandidate = true;
                reasons.Add($"ceilingGap:{topGap:F2}m");
            }
        }

        if (activeCandidate)
        {
            segment.qualityReason = string.Join(", ", reasons);
            return WallCandidateState.ActiveCandidate;
        }

        segment.qualityReason = "accepted";
        return WallCandidateState.Accepted;
    }

    private List<WallSegment> MergeSimilarWallSegments(List<WallSegment> source)
    {
        List<WallSegment> result = new List<WallSegment>();

        foreach (WallSegment s in source)
        {
            bool duplicate = false;

            foreach (WallSegment existing in result)
            {
                if (UVector2.Distance(s.center, existing.center) < 0.25f &&
                    UVector2.Angle(s.direction, existing.direction) < 8.0f)
                {
                    duplicate = true;
                    break;
                }
            }

            if (!duplicate)
            {
                result.Add(s);
            }
        }

        return result;
    }

    private void MoveContainedActiveCandidatesToIgnored(RoomBuildResult result)
    {
        if (!ignoreActiveCandidatesContainedInDetectedWalls ||
            result == null ||
            result.detectedWalls == null ||
            result.detectedWalls.Count == 0 ||
            result.activeWallCandidates == null ||
            result.activeWallCandidates.Count == 0)
        {
            return;
        }

        for (int i = result.activeWallCandidates.Count - 1; i >= 0; i--)
        {
            WallSegment candidate = result.activeWallCandidates[i];

            if (!IsCandidateContainedInAnyDetectedWall(candidate, result.detectedWalls))
            {
                continue;
            }

            candidate.candidateState = WallCandidateState.IgnoredCandidate;
            candidate.qualityReason = string.IsNullOrEmpty(candidate.qualityReason)
                ? "contained in detected wall"
                : candidate.qualityReason + ", contained in detected wall";

            result.activeWallCandidates.RemoveAt(i);
            result.ignoredWallCandidates.Add(candidate);
        }
    }

    private bool IsCandidateContainedInAnyDetectedWall(WallSegment candidate, List<WallSegment> detectedWalls)
    {
        foreach (WallSegment detected in detectedWalls)
        {
            if (IsCandidateContainedInDetectedWall(candidate, detected))
            {
                return true;
            }
        }

        return false;
    }

    private bool IsCandidateContainedInDetectedWall(WallSegment candidate, WallSegment detected)
    {
        if (candidate.length <= 0.001f || detected.length <= 0.001f)
        {
            return false;
        }

        float angle = UVector2.Angle(candidate.direction, detected.direction);
        angle = Mathf.Min(angle, 180.0f - angle);

        if (angle > containedCandidateMaxAngleDegrees)
        {
            return false;
        }

        UVector2 detectedDirection = detected.direction;
        UVector2 detectedNormal = new UVector2(-detectedDirection.y, detectedDirection.x);
        float startDistance = Mathf.Abs(UVector2.Dot(candidate.start - detected.start, detectedNormal));
        float endDistance = Mathf.Abs(UVector2.Dot(candidate.end - detected.start, detectedNormal));
        float maxDistance = Mathf.Max(0.0f, containedCandidateMaxDistanceMeters);

        if (startDistance > maxDistance || endDistance > maxDistance)
        {
            return false;
        }

        float detectedLength = detected.length;
        float padding = Mathf.Max(0.0f, containedCandidateEndpointPaddingMeters);
        float candidateStartT = UVector2.Dot(candidate.start - detected.start, detectedDirection);
        float candidateEndT = UVector2.Dot(candidate.end - detected.start, detectedDirection);

        float minT = Mathf.Min(candidateStartT, candidateEndT);
        float maxT = Mathf.Max(candidateStartT, candidateEndT);

        return minT >= -padding && maxT <= detectedLength + padding;
    }

    private List<WallSegment> BuildClosureEdges(
        List<WallSegment> walls,
        float maxGap,
        float floorY,
        float ceilingY)
    {
        List<WallSegment> closures = new List<WallSegment>();

        if (walls.Count < 2)
        {
            return closures;
        }

        List<UVector2> endpoints = new List<UVector2>();

        foreach (WallSegment wall in walls)
        {
            endpoints.Add(wall.start);
            endpoints.Add(wall.end);
        }

        bool[] used = new bool[endpoints.Count];

        for (int i = 0; i < endpoints.Count; i++)
        {
            if (used[i])
            {
                continue;
            }

            int nearest = -1;
            float nearestDistance = float.MaxValue;

            for (int j = i + 1; j < endpoints.Count; j++)
            {
                if (used[j])
                {
                    continue;
                }

                float d = UVector2.Distance(endpoints[i], endpoints[j]);

                if (d < nearestDistance)
                {
                    nearestDistance = d;
                    nearest = j;
                }
            }

            if (nearest >= 0 && nearestDistance > 0.05f && nearestDistance <= maxGap)
            {
                used[i] = true;
                used[nearest] = true;

                closures.Add(new WallSegment
                {
                    start = endpoints[i],
                    end = endpoints[nearest],
                    center = (endpoints[i] + endpoints[nearest]) * 0.5f,
                    bottomY = floorY,
                    topY = ceilingY,
                    height = Mathf.Max(0.1f, ceilingY - floorY),
                    sourceMeshIndex = -1,
                    sourceMeshName = "ClosureEdge",
                    isClosure = true
                });
            }
        }

        return closures;
    }

    private bool IsClosedEnough(List<WallSegment> walls, List<WallSegment> closures, float maxGap)
    {
        if (walls.Count < 3)
        {
            return false;
        }

        List<UVector2> endpoints = new List<UVector2>();

        foreach (WallSegment wall in walls)
        {
            endpoints.Add(wall.start);
            endpoints.Add(wall.end);
        }

        foreach (WallSegment closure in closures)
        {
            endpoints.Add(closure.start);
            endpoints.Add(closure.end);
        }

        int openCount = 0;

        for (int i = 0; i < endpoints.Count; i++)
        {
            float nearest = float.MaxValue;

            for (int j = 0; j < endpoints.Count; j++)
            {
                if (i == j)
                {
                    continue;
                }

                nearest = Mathf.Min(nearest, UVector2.Distance(endpoints[i], endpoints[j]));
            }

            if (nearest > maxGap)
            {
                openCount++;
            }
        }

        return openCount <= 2;
    }

    private void DisplayRoomBuildResult(RoomBuildResult result)
    {
        if (result == null)
        {
            return;
        }

        if (!float.IsNaN(result.floorY) && !float.IsNaN(result.ceilingY) &&
            TryBuildAutoRoomPolygon(result, out List<UVector2> roomPolygon))
        {
            if (showReconstructedFloor)
            {
                BuildAutoRoomHorizontalSurface("DetectedFloor", result.floorY, true, roomPolygon, floorMaterial);
            }

            if (showReconstructedCeiling)
            {
                BuildAutoRoomHorizontalSurface("DetectedCeiling", result.ceilingY, false, roomPolygon, ceilingMaterial);
            }
        }

        if (showDetectedWallSegments)
        {
            foreach (WallSegment wall in result.detectedWalls)
            {
                CreateWallSegmentObject(wall, result.floorY, result.ceilingY, wallMaterial, "DetectedWall");
            }
        }

        if (showActiveWallCandidates)
        {
            Material candidateMaterial = GetActiveWallCandidateMaterial();

            foreach (WallSegment wall in result.activeWallCandidates)
            {
                CreateWallSegmentObject(wall, result.floorY, result.ceilingY, candidateMaterial, "ActiveWallCandidate", true);
            }
        }

        if (showIgnoredWallCandidates)
        {
            Material ignoredMaterial = GetIgnoredWallCandidateMaterial();

            foreach (WallSegment wall in result.ignoredWallCandidates)
            {
                CreateWallSegmentObject(wall, result.floorY, result.ceilingY, ignoredMaterial, "IgnoredWallCandidate", true);
            }
        }

        if (showClosureEdges)
        {
            foreach (WallSegment edge in result.closureEdges)
            {
                CreateWallSegmentObject(edge, result.floorY, result.ceilingY, closureEdgeMaterial, "ClosureEdge");
            }
        }

    }

    private bool TryBuildAutoRoomPolygon(RoomBuildResult result, out List<UVector2> polygon)
    {
        polygon = new List<UVector2>();

        if (result == null)
        {
            return false;
        }

        List<UVector2> points = new List<UVector2>();

        foreach (WallSegment wall in result.detectedWalls)
        {
            points.Add(wall.start);
            points.Add(wall.end);
        }

        foreach (WallSegment edge in result.closureEdges)
        {
            points.Add(edge.start);
            points.Add(edge.end);
        }

        if (points.Count < 3)
        {
            return false;
        }

        polygon = BuildConvexHull(points);
        return polygon.Count >= 3;
    }

    private void BuildAutoRoomHorizontalSurface(
        string objectName,
        float y,
        bool normalUp,
        List<UVector2> polygon,
        Material material)
    {
        if (polygon == null || polygon.Count < 3)
        {
            return;
        }

        if (SignedArea2D(polygon) < 0.0f)
        {
            polygon = new List<UVector2>(polygon);
            polygon.Reverse();
        }

        Mesh mesh = new Mesh();
        List<UVector3> vertices = new List<UVector3>();
        List<int> triangles = new List<int>();

        foreach (UVector2 p in polygon)
        {
            vertices.Add(new UVector3(p.x, y, p.y));
        }

        for (int i = 1; i < polygon.Count - 1; i++)
        {
            if (normalUp)
            {
                triangles.Add(0);
                triangles.Add(i + 1);
                triangles.Add(i);
            }
            else
            {
                triangles.Add(0);
                triangles.Add(i);
                triangles.Add(i + 1);
            }
        }

        mesh.SetVertices(vertices);
        mesh.SetTriangles(triangles, 0);
        mesh.RecalculateNormals();
        mesh.RecalculateBounds();

        GameObject surface = new GameObject(objectName);
        surface.transform.SetParent(roomRoot != null ? roomRoot.transform : transform, true);

        MeshFilter filter = surface.AddComponent<MeshFilter>();
        MeshRenderer renderer = surface.AddComponent<MeshRenderer>();

        filter.sharedMesh = mesh;
        renderer.sharedMaterial = material != null ? material : GetMaterialForKind(SceneObjectKind.Floor);
    }

    private static List<UVector2> BuildConvexHull(List<UVector2> source)
    {
        List<UVector2> points = source
            .GroupBy(p => new Vector2Int(Mathf.RoundToInt(p.x * 1000.0f), Mathf.RoundToInt(p.y * 1000.0f)))
            .Select(g => g.First())
            .OrderBy(p => p.x)
            .ThenBy(p => p.y)
            .ToList();

        if (points.Count <= 3)
        {
            return points;
        }

        List<UVector2> lower = new List<UVector2>();
        foreach (UVector2 p in points)
        {
            while (lower.Count >= 2 && Cross(lower[lower.Count - 2], lower[lower.Count - 1], p) <= 0.0f)
            {
                lower.RemoveAt(lower.Count - 1);
            }
            lower.Add(p);
        }

        List<UVector2> upper = new List<UVector2>();
        for (int i = points.Count - 1; i >= 0; i--)
        {
            UVector2 p = points[i];
            while (upper.Count >= 2 && Cross(upper[upper.Count - 2], upper[upper.Count - 1], p) <= 0.0f)
            {
                upper.RemoveAt(upper.Count - 1);
            }
            upper.Add(p);
        }

        lower.RemoveAt(lower.Count - 1);
        upper.RemoveAt(upper.Count - 1);
        lower.AddRange(upper);
        return lower;
    }

    private static float Cross(UVector2 a, UVector2 b, UVector2 c)
    {
        UVector2 ab = b - a;
        UVector2 ac = c - a;
        return ab.x * ac.y - ab.y * ac.x;
    }

    private static float SignedArea2D(List<UVector2> polygon)
    {
        float area = 0.0f;

        for (int i = 0; i < polygon.Count; i++)
        {
            UVector2 a = polygon[i];
            UVector2 b = polygon[(i + 1) % polygon.Count];
            area += a.x * b.y - b.x * a.y;
        }

        return area * 0.5f;
    }

    private void CreateWallSegmentObject(
        WallSegment segment,
        float floorY,
        float ceilingY,
        Material material,
        string name,
        bool removeCollider = false)
    {
        GameObject go = GameObject.CreatePrimitive(PrimitiveType.Cube);
        go.name = name;

        UVector2 mid2 = (segment.start + segment.end) * 0.5f;
        float length = UVector2.Distance(segment.start, segment.end);
        float height = Mathf.Max(0.1f, ceilingY - floorY);

        UVector2 dir2 = (segment.end - segment.start).normalized;
        float yaw = Mathf.Atan2(dir2.x, dir2.y) * Mathf.Rad2Deg;

        go.transform.position = new UVector3(
            mid2.x,
            floorY + height * 0.5f,
            mid2.y
        );

        go.transform.rotation = Quaternion.Euler(0.0f, yaw, 0.0f);
        go.transform.localScale = new UVector3(0.04f, height, length);

        if (roomRoot != null)
        {
            go.transform.SetParent(roomRoot.transform, true);
        }

        MeshRenderer renderer = go.GetComponent<MeshRenderer>();

        if (renderer != null && material != null)
        {
            renderer.sharedMaterial = material;
        }

        if (segment.isClosure || removeCollider)
        {
            Collider col = go.GetComponent<Collider>();

            if (col != null)
            {
                Destroy(col);
            }
        }

        AttachSourceMeshDebugChild(go, segment);
    }

    private void AttachSourceMeshDebugChild(GameObject wallObject, WallSegment segment)
    {
        if (!attachSourceMeshDebugChild ||
            wallObject == null ||
            segment.isClosure ||
            segment.sourceMeshIndex < 0 ||
            segment.sourceMeshIndex >= extractedSceneMeshes.Count)
        {
            return;
        }

        ExtractedSceneMesh source = extractedSceneMeshes[segment.sourceMeshIndex];

        if (source == null || source.sourceMesh == null)
        {
            return;
        }

        GameObject sourceObject = new GameObject(
            string.IsNullOrEmpty(segment.sourceMeshName)
                ? $"SourceSceneMesh_{segment.sourceMeshIndex}"
                : segment.sourceMeshName
        );

        MeshFilter meshFilter = sourceObject.AddComponent<MeshFilter>();
        meshFilter.sharedMesh = source.sourceMesh;

        MeshRenderer meshRenderer = sourceObject.AddComponent<MeshRenderer>();
        Material sourceMaterial = GetMaterialForKind(source.kind);

        if (sourceMaterial != null)
        {
            meshRenderer.sharedMaterial = sourceMaterial;
        }

        SetUnityTransformFromMatrix4x4(sourceObject.transform, source.localToWorldMatrix, false);
        sourceObject.transform.SetParent(wallObject.transform, true);
        sourceObject.SetActive(false);
    }

    private Material GetActiveWallCandidateMaterial()
    {
        if (activeWallCandidateMaterial != null)
        {
            return activeWallCandidateMaterial;
        }

        if (runtimeActiveWallCandidateMaterial == null)
        {
            runtimeActiveWallCandidateMaterial = CreateRuntimeColorMaterial(
                "Runtime_ActiveWallCandidate_Material",
                new Color(1.0f, 0.78f, 0.1f, 0.45f)
            );
        }

        return runtimeActiveWallCandidateMaterial != null ? runtimeActiveWallCandidateMaterial : wallMaterial;
    }

    private Material GetIgnoredWallCandidateMaterial()
    {
        if (ignoredWallCandidateMaterial != null)
        {
            return ignoredWallCandidateMaterial;
        }

        if (runtimeIgnoredWallCandidateMaterial == null)
        {
            runtimeIgnoredWallCandidateMaterial = CreateRuntimeColorMaterial(
                "Runtime_IgnoredWallCandidate_Material",
                new Color(1.0f, 0.2f, 0.15f, 0.35f)
            );
        }

        return runtimeIgnoredWallCandidateMaterial != null ? runtimeIgnoredWallCandidateMaterial : wallMaterial;
    }

    private static Material CreateRuntimeColorMaterial(string materialName, Color color)
    {
        Shader shader = Shader.Find("Universal Render Pipeline/Lit");

        if (shader == null)
        {
            shader = Shader.Find("Standard");
        }

        if (shader == null)
        {
            return null;
        }

        Material material = new Material(shader)
        {
            name = materialName,
            color = color
        };

        return material;
    }

    private Mesh GenerateUnityMesh(SceneMesh sceneMesh)
    {
        if (sceneMesh == null)
        {
            return null;
        }

        int triangleIndexCount = (int)sceneMesh.TriangleIndexCount;
        int vertexCount = (int)sceneMesh.VertexCount;

        uint[] indices = new uint[triangleIndexCount];
        sceneMesh.GetTriangleIndices(indices);

        NumVector3[] positions = new NumVector3[vertexCount];
        sceneMesh.GetVertexPositions(positions);

        UVector3[] unityVertices = new UVector3[positions.Length];
        int[] unityIndices = new int[indices.Length];

        for (int i = 0; i < positions.Length; i++)
        {
            unityVertices[i] = new UVector3(
                positions[i].X,
                positions[i].Y,
                -positions[i].Z
            );
        }

        // 이전 방식:
        // triangle winding을 뒤집지 않는다.
        for (int i = 0; i < indices.Length; i++)
        {
            unityIndices[i] = (int)indices[i];
        }

        Mesh mesh = new Mesh();

        if (unityVertices.Length > 65535)
        {
            mesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt32;
        }

        mesh.vertices = unityVertices;
        mesh.triangles = unityIndices;
        mesh.RecalculateNormals();
        mesh.RecalculateBounds();

        return mesh;
    }

    private Material GetMaterialForKind(SceneObjectKind kind)
    {
        switch (kind)
        {
            case SceneObjectKind.Wall:
                return wallMaterial;

            case SceneObjectKind.Floor:
                return floorMaterial;

            case SceneObjectKind.Ceiling:
                return ceilingMaterial;

            default:
                return wallMaterial;
        }
    }

    private void UpdateCameraFloorDebugValues()
    {
        Camera mainCamera = Camera.main;

        if (mainCamera != null)
        {
            lastCameraY = mainCamera.transform.position.y;
        }

        if (sceneRoot != null)
        {
            lastSceneRootY = sceneRoot.position.y;
        }

        if (currentRoom != null &&
            !float.IsNaN(currentRoom.floorY) &&
            !float.IsNaN(currentRoom.ceilingY))
        {
            lastFloorWorldY = currentRoom.floorY;
            lastCeilingWorldY = currentRoom.ceilingY;

            lastEyeToFloor = lastCameraY - lastFloorWorldY;
            lastEyeToCeiling = lastCeilingWorldY - lastCameraY;

            if (sceneRoot != null)
            {
                Vector3 floorWorldPoint = new Vector3(0.0f, currentRoom.floorY, 0.0f);
                Vector3 ceilingWorldPoint = new Vector3(0.0f, currentRoom.ceilingY, 0.0f);

                lastFloorLocalY = sceneRoot.InverseTransformPoint(floorWorldPoint).y;
                lastCeilingLocalY = sceneRoot.InverseTransformPoint(ceilingWorldPoint).y;
            }
        }
    }

    private void LogSceneSummary(Scene scene, RoomBuildResult room)
    {
        int wallCount = scene.SceneObjects.Count(o => o.Kind == SceneObjectKind.Wall);
        int floorCount = scene.SceneObjects.Count(o => o.Kind == SceneObjectKind.Floor);
        int ceilingCount = scene.SceneObjects.Count(o => o.Kind == SceneObjectKind.Ceiling);
        int platformCount = scene.SceneObjects.Count(o => o.Kind == SceneObjectKind.Platform);
        int worldCount = scene.SceneObjects.Count(o => o.Kind == SceneObjectKind.World);

        Debug.Log(
            $"[SU] Objects - Wall:{wallCount}, Floor:{floorCount}, Ceiling:{ceilingCount}, Platform:{platformCount}, World:{worldCount}\n" +
            $"[Room] Walls:{room.detectedWalls.Count}, ClosureEdges:{room.closureEdges.Count}, ClosedEnough:{room.isClosedEnough}, " +
            $"FloorY:{room.floorY:F2}, CeilingY:{room.ceilingY:F2}, Height:{(room.ceilingY - room.floorY):F2}, " +
            $"CameraY:{lastCameraY:F2}, EyeToFloor:{lastEyeToFloor:F2}, Compute:{lastComputeTimeSeconds:F2}s"
        );
    }

    private static float Median(List<float> values)
    {
        if (values == null || values.Count == 0)
        {
            return 0.0f;
        }

        values.Sort();
        int mid = values.Count / 2;

        if (values.Count % 2 == 0)
        {
            return (values[mid - 1] + values[mid]) * 0.5f;
        }

        return values[mid];
    }

    private static UVector2 Average(List<UVector2> points)
    {
        UVector2 sum = UVector2.zero;

        foreach (UVector2 p in points)
        {
            sum += p;
        }

        return sum / Mathf.Max(1, points.Count);
    }

    private static UMatrix4x4 ConvertRightHandedMatrix4x4ToLeftHanded(
        NumMatrix4x4 matrix)
    {
        UMatrix4x4 unityMatrix = new UMatrix4x4();

        unityMatrix.m00 = matrix.M11;
        unityMatrix.m01 = matrix.M21;
        unityMatrix.m02 = -matrix.M31;
        unityMatrix.m03 = matrix.M41;

        unityMatrix.m10 = matrix.M12;
        unityMatrix.m11 = matrix.M22;
        unityMatrix.m12 = -matrix.M32;
        unityMatrix.m13 = matrix.M42;

        unityMatrix.m20 = -matrix.M13;
        unityMatrix.m21 = -matrix.M23;
        unityMatrix.m22 = matrix.M33;
        unityMatrix.m23 = -matrix.M43;

        unityMatrix.m30 = matrix.M14;
        unityMatrix.m31 = matrix.M24;
        unityMatrix.m32 = -matrix.M34;
        unityMatrix.m33 = matrix.M44;

        return unityMatrix;
    }

    private static void SetUnityTransformFromMatrix4x4(
        Transform target,
        UMatrix4x4 matrix,
        bool local)
    {
        UVector3 position = matrix.GetColumn(3);
        Quaternion rotation = matrix.rotation;
        UVector3 scale = matrix.lossyScale;

        if (local)
        {
            target.localPosition = position;
            target.localRotation = rotation;
            target.localScale = scale;
        }
        else
        {
            target.SetPositionAndRotation(position, rotation);
            target.localScale = scale;
        }
    }
}

[Serializable]
public class RoomBuildResult
{
    public List<WallSegment> detectedWalls = new List<WallSegment>();
    public List<WallSegment> closureEdges = new List<WallSegment>();
    public List<WallSegment> activeWallCandidates = new List<WallSegment>();
    public List<WallSegment> passiveWallCandidates = new List<WallSegment>();
    public List<WallSegment> ignoredWallCandidates = new List<WallSegment>();
    public float floorY;
    public float ceilingY;
    public bool isClosedEnough;
}

[Serializable]
public enum WallCandidateState
{
    Accepted,
    ActiveCandidate,
    PassiveCandidate,
    IgnoredCandidate
}

[Serializable]
public struct WallSegment
{
    public UVector2 start;
    public UVector2 end;
    public UVector2 center;
    public float bottomY;
    public float topY;
    public float height;
    public float xzThickness;
    public float aspectRatio;
    public float directionConfidence;
    public WallCandidateState candidateState;
    public string qualityReason;
    public int sourceMeshIndex;
    public string sourceMeshName;
    public bool isClosure;

    public float length => UVector2.Distance(start, end);

    public UVector2 direction
    {
        get
        {
            UVector2 d = end - start;
            return d.sqrMagnitude > 0.0001f ? d.normalized : UVector2.right;
        }
    }
}
